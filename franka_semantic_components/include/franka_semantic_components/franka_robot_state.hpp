...
class FrankaRobotState
 ...
 public:
  ...
  /**
   * @brief Get the the stiffness frame_id
   *
   * @return std::string Name of the stiffness frame
   */
  auto get_stiffness_frame_name() const -> std::string;
 ...
 private:
  ...
  std::vector<std::string> joint_names, link_names_;
